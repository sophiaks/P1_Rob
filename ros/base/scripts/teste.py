...
def roda_todo_frame(imagem):
    ...
    try:
        ...
        media, centro, maior_contorno_area = cormodule.identifica_cor(cv_image, lista_quero[0])
...
if __name__ == "__main__":
    ...
    topico_imagem = "/camera/rgb/image_raw"
    ...
    try:
        ...
        while not rospy.is_shutdown():
            if cv_image is not None:
                mov.centraliza(ptom[0], 20, cv_image)
                if pegou:
                    mov.centraliza_obj(lista_quero, cv_image, resultados)
...
#__________________________________________________________________ACHOU UM ID MAS NÃO É O DA LISTA________________________________________________________________________#
                    ...
                    if len(ptom) > 0:  # Se tiver um ponto de fuga
                        mov.centraliza(ptom[0], 15, cv_image)
                    else:
